...
#        ·  ͼgam.t
for(i in 1:9){
plot(x=c(1:n.mcmc),y=result$gam.t[,i],type='l')
}

#      ֵͼgam.t
par(mfrow=c(2,5)); 
for(i in 1:9){
ergodic1_beta=ergodic_mean(result$gam.t[,i]);
ergodic1_beta_b=ergodic_mean(result$gam.t[,i]);
ylims0=range(c(ergodic1_beta,ergodic1_beta_b))
step=1;
index3=seq(1,n.mcmc,step);
plot(index3,ergodic1_beta[index3],type="l",ylim=ylims0)
...
par(mfrow=c(1,2)); 
plot(colMeans(mean.lam0.t),type="l",col="blue",ylab='baseline function',ylim=c(0,10))
lines(lam0.true.t,type="l",col="red",ylab='baseline function')
